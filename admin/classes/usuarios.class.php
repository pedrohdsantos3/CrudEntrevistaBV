...
class Usuario
{
	...
	public function Cad_usuario()
	{
		...
		if($validaid = $this->ValidaId() == TRUE){
			...
			if($validaEdicao == TRUE){
				...
				SET usu_nome = :nome, usu_email = :login, usu_direitos = :nivel, usu_senha = :senha
				WHERE usu_id = :id");
			...
			$validarCad = $this->Valida_usuario();
				if($validarCad == false){
						...
						$ex = $pdo->prepare("INSERT INTO usuarios (usu_id, usu_nome,
								usu_direitos, usu_email, usu_senha)
								VALUES (:id, :nome, :nivel, :login, :senha)");
	...
	public function Ret_porLogin($usr)
	{
		...
		$ex = $pdo->prepare("SELECT * FROM usuarios WHERE usuario_login = '$usr'");
		$ex->execute();
	...
	{
		include "../configs/conexao.class.php";
		$ex         = $pdo->prepare("SELECT usu_email FROM usuarios WHERE usu_email = '$this->login'");
		$ex->execute();
